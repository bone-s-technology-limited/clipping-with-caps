CAPS.Selection = function ( low, high ) {
	...
	this.box3 = new THREE.BoxGeometry( 1, 1, 1 );
	this.box = new THREE.BoxGeometry( 1, 1, 1 );

	this.boxMesh3 = new THREE.Mesh( this.box3, CAPS.MATERIAL.cap3 );
...
CAPS.Selection.prototype = {
	...
	setUniforms3: function () {

		var uniforms = CAPS.UNIFORMS3.clipping;
	...
	setValue: function ( axis, value ) {
		...
		var buffer = 0.05;
		//var limit = 14;
		//the original state of box is (-1,-1,-1), (2,2,2)
		var limit = 2;  //front
		var limit2 = 1;  //back

		if ( axis === 'x1' ) {
			//this.limitLow.x = Math.max( -limit, Math.min( this.limitHigh.x-buffer, value ) );
			this.limitLow.x = Math.max( -limit2, Math.min( this.limitHigh.x-buffer, value ) );

		} else if ( axis === 'x2' ) {
			this.limitHigh.x = Math.max( this.limitLow.x+buffer, Math.min( limit, value ) );

		} else if ( axis === 'y1' ) {
			this.limitLow.y = Math.max( -limit2, Math.min( this.limitHigh.y-buffer, value ) );
		} else if ( axis === 'y2' ) {
			this.limitHigh.y = Math.max( this.limitLow.y+buffer, Math.min( limit, value ) );
		} else if ( axis === 'z1' ) {
			this.limitLow.z = Math.max( -limit2, Math.min( this.limitHigh.z-buffer, value ) );
		} else if ( axis === 'z2' ) {
